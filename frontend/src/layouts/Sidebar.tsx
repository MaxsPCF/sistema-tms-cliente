...
export default function Sidebar() {
	...
	return (
		<>
			{sidebarOpen && <div className="fixed inset-0 z-30 bg-gray-900/50 md:hidden" onClick={() => dispatch(closeMobileSidebar())} aria-hidden="true" />}
			...
			>
				<div className="flex h-16 items-center justify-between border-b border-gray-100 px-4">
					<div className="flex items-center gap-2 overflow-hidden">
						{!sidebarCollapsed ? (
							<img className="dark:hidden" src="/images/logo/tms/logo_genesis.jpg" alt="Logo" width="200" height="40" />
						) : (
							<img src="/images/logo/tms/genesis_icono_claro.svg" alt="Logo" width="60" height="60" />
						)}
					</div>
					<button className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 md:hidden" onClick={() => dispatch(closeMobileSidebar())} aria-label="Cerrar menú">
						<X className="h-5 w-5" />
					</button>
				</div>
				...
				<button onClick={() => dispatch(toggleSidebarCollapsed())} className="hidden items-center gap-2 border-t border-gray-100 px-4 py-3 text-xs font-medium text-gray-500 hover:bg-gray-50 md:flex">
