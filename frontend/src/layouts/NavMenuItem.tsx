...
export default function NavMenuItem({ modulo, collapsed, depth = 0 }: NavMenuItemProps) {
	...
	const Icon = useMemo(() => {
		if (!modulo.icono) return Dot;
		return resolveIcon(modulo.icono) ?? Dot;
	}, [modulo.icono]);

	// ¿Tiene hijos que merezcan ser mostrados?
	const visibleChildren = useMemo(() => modulo.children.filter(child => hasVisibleContent(child)), [modulo.children]);
	const hasChildren = visibleChildren.length > 0;

	// ¿Algún descendiente está activo? (profundidad completa)
	const isChildActive = useMemo(() => hasActiveDescendant(modulo, location.pathname), [modulo, location.pathname]);
		...
		return (
			<div>
				...
				>
					...
					<span className={cn("flex-1 text-left", collapsed && "md:hidden")}>{modulo.nombreModulo}</span>
					<ChevronDown className={cn("h-4 w-4 shrink-0 transition-transform", expanded && "rotate-180", collapsed && "md:hidden")} />
				</button>

				{expanded && (
					<div className={cn("mt-1 space-y-1 border-l border-gray-100 pl-3", collapsed ? "md:hidden" : "ml-4")}>
						{visibleChildren
							.slice()
							.sort((a, b) => a.orden - b.orden)
							.map(child => (
								<NavMenuItem key={child.idModulo} modulo={child} collapsed={collapsed} depth={depth + 1} />
							))}
					</div>
				)}
			</div>
